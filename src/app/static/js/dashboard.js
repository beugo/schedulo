...
function renderPlansTable(plans) {
  ...
  plans.forEach((plan) => {
    const tr = document.createElement("tr");
    tr.className =
      "hover:bg-gray-100 dark:hover:bg-dark-secondary cursor-pointer";
    tr.onclick = () => {
...
function renderPosts(posts) {
  ...
  if (!posts.length) {
    ...
    emptyCard.className =
      "w-full bg-white dark:bg-dark-fg border border-gray-200 dark:border-dark-border rounded-lg p-4 shadow text-center text-gray-600 dark:text-gray-400";
    emptyCard.innerHTML = `
  ...
  posts.forEach((post) => {
    ...
    card.className =
      "bg-white dark:bg-dark-fg border border-gray-200 dark:border-dark-fg rounded-lg p-4 shadow-sm hover:shadow cursor-pointer flex flex-col gap-2";
    card.onclick = () =>
      (window.location.href = `/plans/view?id=${post.unit_plan.unitplanid}`);
    ...
    for (let row = 1; row <= 6; row++) {
      for (let col = 1; col <= 4; col++) {
        ...
        const unit = post.unit_plan.units.find(
          (u) => u.col === col && u.row === row,
        );
        if (unit) {
          const nameDiv = document.createElement("div");
          nameDiv.className =
            "font-semibold text-sm text-gray-800 dark:text-white truncate w-full";
          nameDiv.title = unit.unitname;
          nameDiv.textContent = unit.unitname;

          const codeDiv = document.createElement("div");
          codeDiv.className =
            "text-xs text-gray-500 dark:text-gray-400 truncate w-full";
