...
function createUnitDiv(unit) {
  ...
  if (unit.semester1)
    tags.appendChild(tag("Sem 1", "bg-blue-100 text-blue-800"));
  if (unit.semester2)
    tags.appendChild(tag("Sem 2", "bg-green-100 text-green-800"));
...
function checkPrerequisites(unit, placedWithTime) {
  ...
  const groups = unit.prerequisites.split("|").map((g) =>
    g
      .trim()
      .split("+")
      .map((c) => c.trim()),
  ...
  const ok = groups.some((group) =>
    group.every(
      (pr) =>
        placedWithTime[pr] !== undefined && placedWithTime[pr] < currentTime,
    ),
  );
...
async function applyInitialPlan() {
  ...
  if (planId) {
    try {
      const res = await fetch(`${endpoints.getPlan}?id=${planId}`, {
        credentials: "include",
      });
  ...
  (Array.isArray(init.units) ? init.units : Object.values(init.units)).forEach(
    (u) => {
      const key = `${u.row},${u.col}`;
      const cell = document.querySelector(
        `${CELL_SELECTOR}[data-key="${key}"]`,
      );
      if (!cell) return;
...
function applyTemplate(template) {
  ...
  template.forEach(({ unit_code, row, col }) => {
    ...
    if (!unit) return;
    const cell = document.querySelector(
      `${CELL_SELECTOR}[data-key="${row},${col}"]`,
    );
    if (!cell) return;
    cell.innerHTML = "";
    cell.appendChild(createUnitDiv(unit));
...
function dragStartHandler(e) {
  ...
  e.dataTransfer.setData(
    "sourceKey",
    this.closest(CELL_SELECTOR)?.dataset.key || "",
  );
...
function setupDragAndDrop() {
  ...
  DOM.unitList.addEventListener("drop", (e) => {
    ...
    const gridUnit = document.querySelector(
      `${CELL_SELECTOR} .unit[data-code="${code}"]`,
    );
    if (gridUnit) {
    ...
    cell.addEventListener("drop", (e) => {
      ...
      const draggedUnit = sourceKey
        ? document.querySelector(
            `${CELL_SELECTOR}[data-key="${sourceKey}"] .unit[data-code="${code}"]`,
          )
        : null;
      if (draggedUnit) {
        draggedUnit.remove();
        document.querySelector(
          `${CELL_SELECTOR}[data-key="${sourceKey}"]`,
...
function validateSemesterPlacement(cell, unit) {
  ...
  const ok =
    (targetSemester === 1 && unit.semester1) ||
  ...
  if (!ok) {
    ...
    badge.className =
      "semester-warning absolute top-1 right-1 bg-red-500 text-white text-[10px] px-1 rounded shadow";
...
function validatePrereqs() {
  ...
  DROP_ZONES.forEach((cell) => {
    ...
    if (!ok) {
      ...
      const badge = document.createElement("div");
      badge.className =
        "prereq-warning absolute top-1 left-1 text-[10px] bg-red-500 text-white px-1 rounded";
...
function setupSaveHandler() {
  DOM.saveButton.addEventListener("click", () => {
    ...
    if (major === "cs" && !DOM.csCoreSelect.value)
      return createAlert(
        "Please select your final core unit before saving.",
        "error",
      );
    ...
    if (!units.length)
      return createAlert("Please place at least one unit.", "error");
    ...
    const unsatisfied = units.filter(
      (u) => !checkPrerequisites(byCode(u.unit_code), placedWithTime).ok,
    );
    if (unsatisfied.length)
      return createAlert(
        "Please make sure all units have their prerequisites satisfied.",
        "error",
      );
