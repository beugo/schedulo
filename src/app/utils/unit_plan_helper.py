...
def get_plan_core_info(plan_id, user_id):
    plan = (
        db.session.query(UnitPlan)
        .filter_by(id=plan_id, user_id=user_id, is_deleted=False)
        .first()
    )
    if not plan:
        return None
    ...
    upToUnit = (
        db.session.query(UnitPlanToUnit)
        .filter_by(unit_plan_id=plan.id, is_deleted=False)
        .all()
    )
    units = []
    for u in upToUnit:
    ...
    return {"unitplanid": plan.id, "name": plan.name, "units": units}
