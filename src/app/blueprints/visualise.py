...
def unit_data():
    ...
    friend_ids = db.session.query(UserFriend.user_id).filter(
        ...
        UserFriend.is_deleted.is_(False)
    ).union(
    ...
    units = (
        db.session.query(
            Unit,
            db.func.coalesce(friend_unit_counts.c.count,
                             0).label("friend_use_count"),
            db.func.coalesce(total_unit_counts.c.count,
                             0).label("total_use_count")
        )
