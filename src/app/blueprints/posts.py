...
def refresh_posts():
    if not current_user.is_authenticated: return jsonify({"message": "User not authenticated", "ok": False}), 401
...
def post():
    ...
    alreadyPosted = db.session.query(Post).filter_by(unit_plan_id=unit_plan.id, is_deleted=False).first()
    if alreadyPosted:
